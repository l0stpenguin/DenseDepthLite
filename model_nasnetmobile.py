...
import keras
def create_model_nasnetmobile(existing='', is_halffeatures=True):
    if len(existing) == 0:
        print('Loading base model (NASNetMobile)..')
        ...
        base_model.load_weights('nasnet_mobile_no_top.h5')
        ...
        decoder = upproject(decoder, int(decode_filters / 2), 'up1', concat_with='adjust_avg_pool_1_5') #30, 40, 256
        decoder = upproject(decoder, int(decode_filters / 4), 'up2', concat_with='activation_72') #60, 80, 128
        decoder = upproject(decoder, int(decode_filters / 8), 'up3', concat_with='activation_12') #120, 160, 64
        decoder = upproject(decoder, int(decode_filters / 16), 'up4', concat_with='zero_padding2d_1') #240, 320, 64
