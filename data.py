...
def get_diml_data(batch_size, data_zipfile='diml_data.zip'):
    data = extract_zip(data_zipfile)
    nyu2_train = list((row.split(',') for row in (data['data/diml_outdoor_train.csv']).decode("utf-8").split('\n') if len(row) > 0))
    nyu2_test = list((row.split(',') for row in (data['data/diml_outdoor_test.csv']).decode("utf-8").split('\n') if len(row) > 0))
    shape_rgb = (batch_size, 480, 640, 3)
    shape_depth = (batch_size, 240, 320, 1)
...
def get_megadepth_data(batch_size, data_zipfile='megadepth_data.zip'):
    ...
    return data, megadepth_train, megadepth_test, shape_rgb, shape_depth
...
def get_redweb_train_test_data(batch_size):
    data, redweb_train, redweb_test, shape_rgb, shape_depth = get_redweb_data(batch_size)

    train_generator = RedWeb_BasicAugmentRGBSequence(data, redweb_train, batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)
    test_generator = RedWeb_BasicAugmentRGBSequence(data, redweb_test, batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)

    return train_generator, test_generator

def get_diml_train_test_data(batch_size):
    data, diml_train, diml_test, shape_rgb, shape_depth = get_diml_data(batch_size)

    train_generator = DIML_BasicAugmentRGBSequence(data, diml_train, batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)
    test_generator = DIML_BasicAugmentRGBSequence(data, diml_test, batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)

    return train_generator, test_generator

def get_megadepth_train_test_data(batch_size):
    data, megadepth_train, megadepth_test, shape_rgb, shape_depth = get_megadepth_data(batch_size)

    train_generator = Megadepth_BasicAugmentRGBSequence(data, megadepth_train, batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)
    test_generator = Megadepth_BasicAugmentRGBSequence(data, megadepth_test, batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)

    return train_generator, test_generator

class Megadepth_BasicAugmentRGBSequence(Sequence):
...
class DIML_BasicAugmentRGBSequence(Sequence):
    def __init__(self, data, dataset, batch_size, shape_rgb, shape_depth, is_flip=False, is_addnoise=False, is_erase=False):
        self.data = data
        self.dataset = dataset
        self.policy = BasicPolicy( color_change_ratio=0.50, mirror_ratio=0.50, flip_ratio=0.0 if not is_flip else 0.2, 
